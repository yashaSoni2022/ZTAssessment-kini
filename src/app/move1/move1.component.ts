...
export class Move1Component {
  ...
  boldTxt:boolean = false;
  italicTxt:boolean = false;
  underlineTxt:boolean = false
  choices = [{
    selected: true,
    name: "choice1",
    id: 1
  ...
  boldText() {
    const choice: any = document.getElementsByClassName("choice-style");
    if (!this.boldTxt) {
      Array.from(choice).forEach((element: any) => {
